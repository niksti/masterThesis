...
\[P_{dishonestBuyer} = P_{B}(Tx_{cheatingseller}) + U_B(G)\]
...
HIER: DISHONEST STRATEGY WINS BECAUSE OF 5.14! WENN ER DAS POSTET, POSTET DER RATIONAL BUYER HONESTB UND DANN IST DER PAYOUT VON SELLER GRÖßER ALS WENN ER HONEST WÄR. DAS WIDERSPRICHT EINER ASSUMPTION:WELCHE? ES DARF SICH NICHT RENTIEREN ZU SCHUMMELN. BZW VIELLEICHT AUCH SCHON 5.9
...
\minisec{It should not be favourable to behave dishonest}
If the Buyer is behaving rational (maximizing his output), then no dishonest seller exists that gets a higher output than the honest seller. (Maybe new assumption??)
Another (valid??) assumption: If the other party behaves rational, behaving honestly yields the best payout. TODO write that into Formulations.\newline


DEFINE $best_B()$, welches ist beste Strategie/Transaktion auf antwort von Seller strategie. Eigentlich als Funktion von Sellers Strategie.
LAUT 5.13
\begin{equation}
best_B(honestTx_s) = honestTx_B
\end{equation}
Mit assumption dass honest seller größeren payout als dishonest seller hat, wenn Buyer rational ist.
\begin{equation}
   SC_S(honest_S,honest_B)- U_S(G) > SC_S(honest_S,best_B(honestTx_B))
\end{equation}
MIT 5.16
\begin{equation}
   SC_S(honest_S,honest_B)- U_S(G) > SC_S(honest_S,honest_B)
\end{equation} 
UND SOMIT
\begin{equation}
U_S(G) < 0
\end{equation}
WAS UNSERE ASSUMPTIONS WIDERSPRICHT
