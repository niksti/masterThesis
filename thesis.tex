...
\niknote{I was working on that while we talked, so NOT finished}
...
\paragraph{Theorem:}There is no function $assign\_coins\_real' :: (5 inputs) -> (2 outputs)$, such that
for all $\forall bc, sc, p\in \mathbb{N}, \forall ss,bs,sh \in \{0,1\}$,\newline

$assign\_coins\_real$(buyercol $=bc$ sellercol $=sc$, price $=p$, sellersent $=ss$, buyerRecv $=bs$) $= assign\_coins\_ideal$(buyercol=$bc$ sellercol=sc, price=p, sellersent=ss, buyerRecv=bs ,shipped=s))\newline

with $=$ we mean that assign\_coins' returns a value that is in the ranges defined by the ideal functionality.
\paragraph{Proof:}\label{sec:proof} We look at the case where $sellercol=5,Buyercol=5,price=4,sellersent=1buyerRecv=0,shipped=1$.
By definition of the ideal functionality (c.f. figure~\ref{fig:payout-ideal-sc}), its payoff to the Buyer has to be:
\begin{equation}
    assign_coins_ideal
\end{equation}
