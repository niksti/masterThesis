...
\definecolor{sellercolor}{named}{red}
\definecolor{buyercolor}{named}{blue}
...
%In the most cases cryptocurrencies are based on blockchain technology. Due to the immutable nature of a blockchain, payment transactions by using cryptocurrencies are vulnerable for scams.
 ...
 %   After iterative approach, proof that no solution, problem modification, solution for modified problem.
%proof that no solution exists for initial problem and solution to modified problem
...
%The goal of this project is to answer this question positively. We propose a scheme in which the parties lock on-chain some collateral which will only be released if they agree on-chain on the success or failure of the sale, otherwise burnt. To avoid coercion, after sending the payment the Buyer ignores messages from the Seller, on receipt of the item asks a trusted (only by the Buyer) party to verify whether the item is the expected good or not and reports the answer to the blockchain. If, before paying, the Buyer can credibly threaten the Seller that she will stick to this strategy after paying, then it is, we believe, provable that coercion is impossible. A number of ideas to achieve such a credible threat while minimizing the needed trust exist
   ...
   %% we assumed, if it is possible, a possible protocol would be found by us in this step.
	...
	%\niknote{Should I remain this abstract or can I go a little bit into detail? (we allow for the protocol to have a third party that helps the Buyer)}
...
\end{equation}
%where $s^*_{-i}$ denote the strategies of all players other than $i$.\newline
\end{definition}
...
%While the hash of a file can be efficiently computed, it is computationally infeasible to compute.
...
%In this chapter we will formalize the problem. We first define the entities that are participating in the game and then we formalize the corresponding game.\newline
...
%The payment $P$ is now routed via the Smart Contract.  
        ...
        child[sibling distance = 9cm]{node[draw]{$S$}
                child[sibling distance = 8cm]{node[draw]{$B$}
                		...
                		child[sibling distance = 4cm]{node[draw]{$S$}
                		    ...
                		    child[sibling distance = 4cm]{node[shape=rectangle]{$P-C-U_S(G)|U_B(G)-P$} edge from parent node [right] {\emph{Ignore}}}edge from parent node [right] {\emph{Coerce}}} edge from parent node [left] {\emph{Send}}} 
                child[sibling distance = 8cm]{node[draw]{$B$}
...
\end{tikzpicture}
 \centering
    \caption{Seller collateral protocol: Both parties report} 
    \label{fig:tree-seller-reports-first-sellercol}
\end{figure}
\newpage
%\captionof{figure}{Only the Seller deposits collateral: Seller reports first}
%\newpage
\begin{table}[htb!]
\centering
    \begin{tabular}{ |c|c| }
...
\end{tikzpicture}
 \centering
    \caption{Only the Seller deposits collateral: Buyer reports first} 
    \label{fig:1234}
\end{figure}
%\captionof{figure}{Only the Seller deposits collateral: Buyer reports first}
%\newpage
...
\textbf{Seller} \<\< \textbf{Smart Contract} \<\< \textbf{Buyer} \pclb
%-----------COLLATERAL-----------------
\pcintertext [dotted] {Deposit Collateral} \\
\< \sendmessageright{top = $C_S$} \< \<  \sendmessageleft{top = $C_B$} \<   \pclb
\pcintertext [dotted] {Send Resources} \\
%-----------SEND P------------------
\< \< \< \sendmessageleft{top = $P$} \< \\
%-----------SEND G------------------------
 \< \sendmessagerightx[13cm]{6}{G} \<\< \pclb
\pcintertext [dotted] {Report to the SC} \\
%-----------SELLER REPORTS----------------------
\< \sendmessageright{top=report 1} \< \< \< \\
\< \< \< \sendmessageleft{top = report 1} \< \\
\<\< 
\dbox{\begin{subprocedure}
\pseudocodeblock[linenumbering]{
\pcif \text{sellerReport == buyerReport}\\
\t \text{pay back collateral}\\
\t \pcif \text{sellerReport == 1}\\
\t \t \text{send $P$ to Seller}\\
\t \pcelse \text{send $P$ to Buyer}\\
%\t \pcelse \text{cut the collateral}\\
\pcelse \text{cut the collateral}
}
\end{subprocedure}}
\<\<\\
\<  \sendmessageleft{top=$P$} \<\<\< \pclb
%---------------CHECK IF G IS RIGHT-------------
%\< \< \< \< \text{$G$ is right?} \< \< \\
\pcintertext [dotted] {Pay back collateral} \\
\<  \sendmessageleft{top=$C_S$} \<\<\< \\
\< \< \< \sendmessageright{top=$C_B$} \<  \\
}
 \centering
...
\begin{figure}[htb!]
\fbox{
 \procedureblock[linenumbering]{Two side collateral: Seller reports first}{
...
\begin{figure}[htb!]
\fbox{
 \procedureblock[linenumbering]{Two side collateral: Buyer reports first}{
    \text{Seller deposits collateral $C_S > P$ to the Smart Contract}\\
...
\begin{equation}
\label{eq:theorem}
    \texttt{assign\_coins\_real(}C_S, C_B, P, R_S, R_B\textt{)} = \texttt{assign\_coins\_ideal(}C_S, C_B, P, R_S, R_B, G\textt{)}
\end{equation}

\niknote{Can we really model this function with a randomness input? In the blockchain and SC environment there is normally no access to randomness right? We also tried to prevent randomness the whole thesis.}
\paragraph{Proof:}\label{sec:proof} 
We provide a proof by contradiction: Let us assume for the sake of contradiction that the Theorem is wrong. That means there exists a function \texttt{assign\_coins\_real}: %$(C_B, C_S, P, S_B, B_S) \to (O_S, O_B)$, 
...
\begin{equation}
\label{eq:ideal_1}
     \texttt{assign\_coins\_ideal(}C_S=42,C_B=42,P=1,R_S=\top,R_B=\bot,G=\top\textt{)}[1] \geq P + C_S = 1 + 42 = 43
\end{equation}

From equation \ref{eq:theorem} and equation \ref{eq:ideal_1} we get:
\begin{equation}
\label{eq:real_1}
     \texttt{assign\_coins\_real(}C_S=42,C_B=42,P=1,R_S=\top,R_B=\bot\textt{)}[1] \geq 43
\end{equation}

We now change the sixth ($G$) input from $\top$ to $\bot$: $C_B=42,C_S=42,P=1,R_S=\top,R_B=\bot,G=\bot$. The payoff from the ideal function (c.f. Figure~\ref{fig:code_ideal_sc}) to the Seller has to be:
\begin{equation}
\label{eq:ideal_2}
    \texttt{assign\_coins\_ideal(}C_S=42,C_B=42,P=1, R_S=\top,R_B=\bot,G=\bot\textt{)}[1] <  C_B = 42
\end{equation}

From equation \ref{eq:theorem} and equation \ref{eq:ideal_2} we get:
\begin{equation}
\label{eq:real_2}
    \texttt{assign\_coins\_real(}C_S=42,C_B=42,P=1,R_S=\top,R_B=\bot\textt{)}[1] <  42
\end{equation}
...
%------------TELL THE MEDIATOR---------------
\< \< \< \< \sendmessageleft{top=description of $G$,length=2cm} \>\>\pclb
%-----------COLLATERAL-----------------
\pcintertext [dotted] {Deposit Collateral} \\
...
\begin{figure}[htb!]
\fbox{
 \procedureblock[linenumbering]{Mediated protocol: honest Buyer behaviour}{
    \text{Tell the right SC address and the right specifications for $G$ to the Mediator}\\
    \text{Deposit $C_B>P$ to the Smart Contract}\\
    \text{Send funds of amount $P$ to the Smart Contract}\\
    \text{Wait and receive $G$ from the Mediator}
    }
}
    \centering
    \caption{Mediated protocol: honest Buyer behaviour}
    \label{pro:honest-buyer-behaviour}
\end{figure}
...
\begin{figure}[htb!]
\fbox{
 \procedureblock[linenumbering]{Mediated protocol: honest Seller behaviour}{
    \text{Deposit $C_S>P$ to the Smart Contract}\\
    \text{Send $G$ to the provided address (which is now the Mediator's)}\\
    \text{Report to the Smart Contract  that $G$ was sent}
    }
}
    \centering
    \caption{Mediated protocol: honest Seller behaviour}
    \label{pro:honest-seller-behaviour}
\end{figure}
...
\minisec{Key results}
 ...
 %After showing that these approaches do not solve the proposed problem,
...
%maybe find a formal way to distinguish working protocols from not working ones, given the new assumptions where we allow a Mediator? probably dependend on the behaviour of the Mediator. why is it working?
%postal office reports: reputation system
