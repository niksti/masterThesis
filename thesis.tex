...
TODO: TREE WHERE BUYER IS NOT RATIONAL\newline
\begin{tikzpicture}[sibling distance=7cm,level distance=6em,every node/.style={shape=circle,align=center}]
    \node[draw]{S}
        child[sibling distance = 9cm]{node[draw]{S}
    		child[sibling distance = 5cm]{node[draw]{B}
...
\end{tabular}
\end{center}
Now what if the Buyer stops acting rational and chooses option 7 over 8?\newline
The Buyer chooses:
\begin{itemize}
    \item 1 over 2 ($U_B(G) > U_B(G) -P -C_B$)
    \item 4 over 3 ($U_B(G) - P > U_B(G) -P -C_B$)
...
PARADOX: It is better to report it as the seller would not try to cheat, but it is not Rational, as he gets lower payout: This can be explained: It is worse for the Buyer to be rational, because the seller knows that he is rational -> will not report him
To counteract this problem we introduce a third party, that is not modeled as rational. The purpose of the third party is to 
