...
\begin{document}
	...
	%% Title page
	%%%%%%%%%%%%%
	\title{Trustless incoercible sale of a physical good over a blockchain}
...
\minisec{Description}On the top are the possible strategies for the buyer: \emph{Pay} is the strategy where the buyer behaves honestly and \emph{Withhold} is the strategy where the Buyer behaves dishonestly.\newline
...
\subsection{Possible strategies} \orfnote{Make the next 2 phrases more precise.
...
To incentivize an entity to behave honestly we introduce collateral. The collateral will be lost if the entity does not behave honestly. In this section we will look at a protocol where only one the Buyer deposits collateral. We introduce a Smart Contract which handles the collateral.\newline
...
\fbox{
 \procedureblock[linenumbering]{Buyer Collateral protocol}{
    ...
    \text{After receiving $G$, the Buyer sends funds of amount $P$ to the seller \orfnote{via the Smart Contract}}\\
    \text{The Smart Contract releases the collateral of the Buyer}
}
}
    ...
    \label{pro:buyer-col}
\end{figure}
\orfnote{Mention that when the Buyer pays, she implicitly
...
With this new protocol there are new possible strategies. Of course there is still the honest strategy for both players (\emph{Send} and \emph{Pay}), where they behave according to the protocol description. With this strategy the trade unravels successfully.\newline
But there exist other strategies: The dishonest strategy is again not sending the resources. This strategy is again denoted as \emph{Withhold} (when the buyer does not send the coins) and \emph{Keep} (when the seller does not send the physical good.\newline
The Buyer can also refuse to pay the collateral to the Smart Contract. This behaviour is denoted as $-C_B$.\newline
When the Seller does not send the physical good in step 2 (plays strategy \emph{Keep}), the Buyer still has to decide if he wants to send $P$ to the seller in order to get back his collateral. Sending $P$ in this scenario is denoted as $P_B$ and not sending $P$ is denoted as $-P_B$.\newline
...
\begin{tikzpicture}[sibling distance=7cm,level distance=6em,every node/.style={shape=circle,align=center}]
      \node[draw]{$B$}
        child{node[draw]{$S$}
                ...
                child[sibling distance = 3cm]{node[shape=rectangle]{$0|-C$}edge from parent node [right] {$-P_B$}}edge from parent node [right] {\emph{Keep}}} edge from parent node [left] {\emph{Pay}}} 
...
\end{figure}
To analyze the Payout, we again start at the bottom at the Buyer's decision. On the left branch the Buyer would choose \emph{Pay}, as $U_B(G) - P > U_B(G) - C$.\newline
On the right branch the Buyer would choose P$_B$, as $-P > -C$.\newline
Based on these decisions the Seller would choose \emph{Keep}, because $P > P - U_S(G)$.\newline
        ...
        child[sibling distance = 9cm]{node[draw]{$S$}
                child[sibling distance = 8cm]{node[draw]{$B$}
                		...
                		child[sibling distance = 4cm]{node[draw]{$S$}
                		    ...
                		    child[sibling distance = 4cm]{node[shape=rectangle]{$P-C-U_S(G)|U_B($B$)-P$} edge from parent node [right] {$-C_S$}}edge from parent node [right] {$C_B$}} edge from parent node [left] {\emph{Send}}} 
                child[sibling distance = 8cm]{node[draw]{$B$}
                    child[sibling distance = 3cm]{node[shape = rectangle]{$P-C|-P$} edge from parent node [left] {\emph{Pay}}} 
                    child[sibling distance = 3cm]{node[draw]{$S$}
                        child[sibling distance = 3cm]{node[shape=rectangle]{0$|$0} edge from parent node [left] {$C_S$}}
                        child[sibling distance = 3cm]{node[shape=rectangle]{$P-C|-P$} edge from parent node [right] {$-C_S$}}edge from parent node [right] {$C_B$}}edge from parent node [right] {\emph{Keep}}} edge from parent node [left] {\emph{Pay}}} 
...
\subsection{Payout: The Buyer reports first}
\begin{tikzpicture}[sibling distance=7cm,level distance=6em,every node/.style={shape=circle,align=center}]
      \node[draw]{$B$}
        child[sibling distance = 9cm]{node[draw]{$S$}
...
The Seller chooses \emph{Keep}, as 
\[0>-U_S(G)\]
...
\end{figure}
The honest and irrational case can be implemented without the last input. There are multiple possible solutions.\newline
...
Similar to section 5.3\niknote{reference, maybe ommit because it is the same}, following protocol is proposed:
\begin{enumerate}
    \item Seller deposits collateral $C_S > P$ to the Smart Contract
    \item Buyer deposits collateral $C_B > P$ and price $P$ to the Smart Contract
...
In the real world, the smart contract does not have the last input (bool:shipped) know
the ground truth.
