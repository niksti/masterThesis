...
\begin{tikzpicture}[sibling distance=7cm,level distance=6em,every node/.style={shape=circle,align=center}]
      \node[draw]{B}
        child[sibling distance = 9cm]{node[draw]{S}
          ...
          child[sibling distance = 7cm]{node[shape = rectangle]{$0|-C$}edge from parent node [right] {$-H_B$}};
\end{tikzpicture}
...
To counteract this problem we introduce a third party, that is not modeled as rational.
