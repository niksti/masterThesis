...
\newcommand{\niknote}[1]{\authnote{\color{red}}{Orfeas}{#1}}
...
To analyze the game, we start at the bottom at the seller's decision. As he would choose the option which yields him the greatest payout, he chooses $Keep$ (Because $P > P - U_S(G)$).\newline
Therefore the Buyers has to choose between a payout of 0 (by choosing $Withhold$) and $-P$ (by choosing $Pay$). Since $0 > -P$, he chooses $Withhold$.\newline
...
\orfnote{mention that this method of finding the Nash Equilibrium is called
"iterated elimination of strictly dominated strategies".}

\subsection{Game: The Seller acts first} \orfnote{apply here as well the notes
...
The analysis is similar to the above case, but the roles are reversed. The Buyer would choose $Withhold$ as $U_B(G) > U_B(G)-P$.\newline
...
\subsection{Game: Both parties act simultaneously} \orfnote{Also apply notes
here}
...
\end{table}
If we look at the payout table we can see that $Withhold$ dominates $Pay$ and $Keep$ dominates $Send$. This means that the dishonest strategy provides a bigger payout regardless of the strategy of the other party. \newline
...
\section{Second Try: One-Sided collateral} \orfnote{rename to "Second Try: One-Sided
                ...
                child[sibling distance = 5cm]{node[draw]{$S$} 
                		...
                		child[sibling distance = 5cm]{node[draw]{$S$} 
                		    ...
                		    child[sibling distance = 2cm]{node[shape = rectangle]{8} edge from parent node [right] {1}}
                		     edge from parent node [right] {1}}
                		     edge from parent node [right] {$Keep$}} edge from parent node [left] {$Pay$}} 
          child[sibling distance = 9cm]{node[shape = rectangle]{0$|$0}edge from parent node [right] {$Withhold$}};
\end{tikzpicture}
\captionof{figure}{Only the Seller deposits collateral: the Buyer reports first}
\begin{table}[htb!]
\centering
...
chooses not to send the physical good ($Keep$).\newline
No matter which decision the Buyer takes at the top of the tree, the payout will be $0|0$ and the trade unsuccessful.
...
As we have shown (8.1) \orfnote{we have to discuss what will be the fate of the
proof}, it is not possible (under our assumptions) to design a protocol that would create a successful game (as described in 4.2). The biggest problem is that we have no reliable way of telling the Smart Contract whether the physical good $G$ was actually shipped or not. If there is a dispute, the Smart Contract does not know which party is lying and has to be punished.\newline
...
\subsubsection{Two Side Collateral with punishing Buyer} \orfnote{instead of
"punishing", I somewhat prefer "truthful". If you agree, change throughout the
text}
...
\orfnote{this figure is too wide to the right.}
\begin{figure}[htb!]
    \centering
    \begin{tikzpicture}[sibling distance=7cm,level distance=6em,every node/.style={shape=circle,align=center}]
    \node[draw]{$S$}
    ...
    \caption{Two Side Collateral with punishing Buyer}
\end{figure}
...
Now a punishing Buyer stops acting rational \orfnote{rationally} and chooses option 7 over 8.\newline
The Buyer chooses:
\begin{itemize}
    \item 1 over 2 ($U_B(G) > U_B(G) -P -C_B$)
    \item 4 over 3 ($U_B(G) - P > U_B(G) -P -C_B$)
...
The answer to this question is no. \orfnote{If the Seller has already misbehaved,} It is more profitable for the Buyer to not report the misbehaviour, as he at least receives back his collateral.\newline
It is not the act of reporting that is profitable for the Buyer, rather it is
the threat of reporting \orfnote{before the Seller chooses whether to misbehave}. If the Seller thinks that maybe the Buyer will report him, he is discouraged of trying to cheat and not sending $G$.\newline
The problem lies in the way we laid the rules for the game: As we defined both players as always behaving rational \orfnote{rationally}, the Seller knows that the Buyer can not report him, because it is not the rational decision.\newline
...
We redesign the game by adding another Entity. The \orfnote{Its} purpose of the newly added Entity is to assist the Buyer in convincing the Seller that he will be punished if he does not send $G$. We need this Entity to maintain the rationality of the Buyer, as a rational Buyer will not report any dishonest behaviour of the Seller.\newline
This new Entity is not modeled as a rational party. It rather wants
\orfnote{don't use the word "wants", in general don't assign agency to the new
Entity. Only rational parties have agency.} to achieve its goal, which is to turn the non credible threat of the Buyer to a credible threat. If this is achieved, a Seller knows that it is unprofitable to behave dishonestly.\newline
The third Entity will be fully \orfnote{not fully, not with its money} trusted
by the Buyer. It has its own physical address and  the ability to receive the
physical good $G$. It also has the ability to evaluate, \orfnote{remove the
comma, this is german} whether the received item actually is the agreed upon item ($G$).\newline
The third Entity also has its own wallet on the blockchain and \orfnote{don't
refer to a wallet} can communicate with the Smart Contract and the Buyer.

\section{Formal definition of the new Entity}
\subsubsection{Adding an Entity}
We introduce another entity, which we call Mediator M \orfnote{use math
mode for $M$} and always has the same behaviour: \orfnote{has the following
behaviour (without always, same)}
\begin{enumerate}
    \item publish his public key and physical address \orfnote{remove this step,
    add as an assumption that the buyer knows this beforehand}
    \item receive SC address and specifications that the \orfnote{remove "the"} describe $G$
    \item receive the physical good
    \item check, \orfnote{comma} if the the physical good match the descriptions
    \orfnote{description in singular} for $G$
    \begin{itemize}
        \item YES \orfnote{if so}: report 1 to the SC and send the good to the Buyer
        \item NO \orfnote{else}: destroy the good and report 0 to the SC
    \end{itemize}
\end{enumerate}

\subsubsection{Assumptions for the new entity} \orfnote{there is a bit of
repetition between here and just before 6.1}
...
\section{New proposed protocol} \orfnote{Rename to e.g. Mediated protocol}
...
The Mediator is always behaving \orfnote{always behaves} in the same way
...
choose the one option, \orfnote{comma} which yields him the biggest payout. This is sending the good ($Send$) and reporting 1, as: 
...
The Friend(F) will receive the physical good $G$ and check if it
\orfnote{matches} the agreed good. After that, he reports 0 (received the good successfully) or 1 (did not receive the good successfully) to the Smart Contract. After that he will send the $G$ to the Buyer.
...
\orfnote{figure exceeds right margin}
\begin{tikzpicture}[sibling distance=7cm,level distance=6em,every node/.style={shape=circle,align=center}]
    \node[draw]{$S$}
        child[sibling distance = 9cm]{node[draw]{$S$}
...
the payout are exactly the same as in the two-sided collateral protocol \orfnote{add
reference to figure}. As the Seller knows that F will act in the best interest of the Buyer, he can still misbehave without being punished.\newline
To solve the problem we need to come up with a Mediator that is fully trusted by
the Buyer, but is not acting in his best interest \orfnote{"acting in the
buyer's best interest" is a contentious way to put it, better change it. In a
sense, the Mediator acts more in the buyer's best interest than the friend.}.
Every entity that is acting in the best interest for the Buyer will not be a
...
\minisec{Protocol}
\begin{enumerate}
    \item MAYBE: Postal Office releases Public Key \orfnote{remove this as discussed}
    \item Buyer gives SC address and specifications for $G$ to the Postal Office
    \item Seller and Buyer deposit Collateral to the SC
    \item Buyer sends $P$ to the SC
    \item Seller hands $G$ to the Postal Office and reports 1/0 \orfnote{up to
    now the protocols were described following the honest scenario, so putting
    1/0 here is inconsistent.} to the SC
...
\begin{enumerate}
    \item it releases the its Pk \orfnote{remove this as discussed}
    \item It receives the SC address and specifications that describe G
    \item It receives the physical good from the Seller
    \item It checks, if the received good matches the specifications for $G$
        \begin{itemize}
            \item YES \orfnote{if/else}: it reports 1 and sends the good to the Buyer
...
\begin{enumerate}
    ...
    \item Seller and Buyer deposit Collateral to the SC
    \item Buyer sends $P$ to the SC
    \item Seller hands $G$ to the Machine and reports 1/0 \orfnote{inconsistent} to the SC
...
\subsubsection{Technology of the machine} \orfnote{Technology $\rightarrow$
realizability}
