...
\end{tikzpicture}\newline
\caption{Naive approach, Buyer sends first}
\end{figure}
%\captionof{figure}{Naive approach, Buyer sends first}

To analyze the game, we start at the bottom at the seller's decision. As he would choose the option which yields him the greatest payout, he chooses $-H_S$ (Because $P > P - U_S(G)$).\newline
...
The Nash Equilibrium of this game is \{$-H_S$, $Withhold$\} and results in a payout of $0|0$. Again, both parties are not sending anything and the trade is unsuccessful.
...
\end{table}
If we look at the payout table we can see that $Withhold$ dominates $H_B$ and $-H_S$ dominates $H_S$. This means that the dishonest strategy provides a bigger payout regardless of the strategy of the other party. \newline
...
\begin{figure}[htb!]
    ...
    \node[draw]{S}
        child[sibling distance = 9cm]{node[draw]{S}
    		...
    		child[sibling distance = 5.5cm]{node[draw]{M}
    		    child[sibling distance = 3cm]{node[shape = rectangle]{$P-U_S(G)| U_B(G)-P$} edge from parent node [right] {1}}edge from parent node [right] {1}}edge from parent node [left] {$H_S$}} 
        child[sibling distance = 9cm]{node[draw]{S}
            ...
            child[sibling distance = 5cm]{node[draw]{M}
    		    child[sibling distance = 3cm]{node[shape = rectangle]{$- C_S| -P-C_B$} edge from parent node [right] {0}}edge from parent node [right] {1}}edge from parent node [right] {$-H_S$}};
    \end{tikzpicture}
...
choose the one option, \orfnote{comma} which yields him the biggest payout. This is sending the good ($H_S$) and reporting 1, as: 
