...
\minisec{Seller}An entity S that owns a specific physical good $G$. The seller
...
\minisec{Buyer}An entity B that owns coins of at least amount of $P$
...
paragraph breaks.}
\minisec{Start Point} \orfnote{rename to Initial State}
...
\minisec{End Point} \orfnote{rename to Desired Final State}
...
\minisec{Goal}
The goal \orfnote{of this work} is to design a protocol in which behaviour that results in a successful trade is a Nash Equilibrium.\newline
...
simultaneous.}
\minisec{Example Tree} \orfnote{for the example, prefer parties $A$, $B$ instead
...
\orfnote{I think the notation $(-)H_{B/S}$ is not helpful. Use full words
"Send"/"Keep" for S and "Pay"/"Withhold" for B, even in the games. There's
space.}
...
\minisec{Payout: The Buyer sends first}\orfnote{E.g. here use
\textbackslash{}subsection instead of \textbackslash{}minisec. Rename to "Game:
Buyer acts first"}
...
To analyze the payout \orfnote{payout $\rightarrow$ game}, we start at the bottom at the seller's decision. As he would choose the option which yields him the greatest payout, he chooses $-H_S$ (Because $P > P - U_S(G)$).\newline
...
\minisec{Payout: The Sender sends first} \orfnote{apply here as well the notes
...
\minisec{Payout: Both parties send simultaneously} \orfnote{Also apply notes
here}
